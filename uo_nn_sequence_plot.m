function uo_nn_sequence_plot(Xds,ydseq,wo)
[n,p]=size(ydseq);
yds=zeros(1,p);
for j=1:p
    if ydseq(j)==1
...
num = zeros(7,5);
%
iwo=1; if size(wo,1)==0, wo=zeros(175,1); iwo=0; end
%
posi = 1;
posf = size(Xds,2);
iplot = 0;
...
for i=range
    if iplot== 0 || (iplot == 1 && round(y(Xds(:,i),wo)) ~= yds(i))
        ...
        if ~iwo
            ...
            if round(y(Xds(:,i),wo)) == yds(i) %si dona el que ha de donar
                if yds(i)==1
                    colormap(im(i),summer);
                    colormap(im(i-1),summer);
                    colormap(im(i-2),summer);
                    colormap(im(i-3),summer);
                    colormap(im(i-4),summer);
                else
                    colormap(im(i),winter);
                end
            else %si no dona el que ha de donar
                if yds(i)==1
